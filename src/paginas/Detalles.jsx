        ...
        <div className="lg:w-1/2 flex flex-col gap-6">
              ...
              <div className="flex items-center gap-2 shrink-0">
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm font-medium transition-all duration-200 ${
                    guardado ? 'bg-orange-50 text-orange-500' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'
                  }`}
                >
                  <Heart size={16} className={guardado ? "fill-orange-500" : "fill-none stroke-current"} />
                ...
                <button 
                  ...
                  disabled={generando}
                  className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm font-medium transition-all duration-200 ${
                    estadoPDF === 'exito' ? 'bg-green-50 text-green-600' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'
                  } ${generando ? 'cursor-wait opacity-70' : 'cursor-pointer'}`}
                >
                  {generando ? <Loader2 className="animate-spin" size={16} /> : estadoPDF === 'exito' ? <CheckCircle size={16} /> : <Download size={16} />}
                  <span className="hidden sm:inline">
                    {generando ? t('detalles.generando') : estadoPDF === 'exito' ? t('detalles.descarga_exitosa') : t('detalles.descargar')}
                  </span>
                </button>
              </div>
            </div>
            <p className="text-gray-500 leading-relaxed">{destinoMostrado?.descripcion}</p>
          </div>
          ...
          <div className="flex items-center gap-4">
            <div className="bg-orange-50 rounded-2xl px-6 py-4 flex-1 text-center">
              ...
              <p className="text-3xl font-bold text-orange-500">$ {destinoMostrado?.presupuesto}</p>
            </div>
            <div className="bg-gray-50 rounded-2xl px-6 py-4 flex-1 text-center">
              ...
              <p className="text-3xl font-bold text-gray-800">{destinoMostrado?.calificacion}<span className="text-base font-normal text-gray-400"> / 5</span></p>
              <p className="text-xs text-gray-400 mt-1">{destinoMostrado?.cantidadVotos} {t('detalles.votos_comunidad')}</p>
            </div>
          </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {destinoMostrado.alojamiento.map((a, i) => (
                  <Tarjeta
                    key={i}
                    destino={{ nombre: a.nombre, descripcion: a.reseña, presupuesto: a.presupuesto, pais: a.ubicacion }}
                    tipo="alojamiento"
                  />
                ))}
              </div>
